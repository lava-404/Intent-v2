...
pub fn open_intent(ctx: Context<OpenIntent>, token_a_amt: u64, recipient: Pubkey, expiry: i64, token_b_amt_min: u64, token_b_mint: Pubkey) -> Result<()> {


  let intent_account = &mut ctx.accounts.intent_account;
  let created_at = Clock::get()?.unix_timestamp;

  require!(token_a_amt > 0, ErrorCode::InvalidAmount);
  require!(token_b_amt_min > 0, ErrorCode::InvalidMinOutput);
  require!(expiry > created_at, ErrorCode::InvalidExpiry);

  require!(
    expiry - created_at <= ctx.accounts.protocol_config.max_intent_duration,
